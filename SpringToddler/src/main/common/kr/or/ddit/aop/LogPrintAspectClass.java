...
public class LogPrintAspectClass {
	...
	// 메서드(joinpoint), 메서드들(pointcut) 대상 호출 전과 종료 후(advice) 주입 될 공통 코드(aspect)의 실행(weaving)
		public Object theWholeJoinpointCallAround(ProceedingJoinPoint joinPoint) throws Throwable {
			// 해당 메서드(joinpoint)가 포함된 빈 클래스 정보
			String beanClassName = joinPoint.getTarget().getClass().getName(); // joinpoint가 포함된 빈 클래스 네임을 취득
			String beanMethodName = joinPoint.getSignature().getName(); // 메소드 명 취득
			
			logger.debug("target Bean Class : {} | joinpoint : {}호출 되고....", beanClassName, beanMethodName);
			
			// 상단 코드 : 해당 메서드(joinpoint) 호출 전 실행
			Object methodReturnValue = joinPoint.proceed();
			// 하단 코드 : 해당 메서드(joinpoint) 종료 후 실행 
			
			logger.debug("target Bean Class : {} | joinpoint : {}가 종료됩니다.", beanClassName, beanMethodName);
