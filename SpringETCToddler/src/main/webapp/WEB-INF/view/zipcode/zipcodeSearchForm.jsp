...
$(function(){
	$('input[type=image]').click(function(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath}/zipcode/zipcodeSearch.do',
			dataType:'json',
			data:{ dong : $('#dong').val() },
			...
			success:function(result){
				var htmls = '<table width="100%" height="190" border="0" cellspacing="0"'+
		           'cellpadding="0" style="overflow:scroll;">' +
		           '<thead>' +
		           	  '<tr>' +
		           	      '<th>우편번호</th>'+
		           	      '<th>시도</th>'+
				...
				for(var i=0; i<result.length; i++){
					var zc = result[i].zipcode;
					var addr = result[i].sido + ' ' + 
							   result[i].gugun + ' ' +
							   result[i].dong;
// 					'<td><a href="javascript:zipCodeEvt(\''+zc+'\',\''+addr+'\');">'+
//                     result[i].zipcode+'</a></td>'+
					htmls += '<tr>'+                       
				...
				$('table table table tr:gt(0)').click(function(){
					...
					var address = $(this).find('td:eq(1)').text() + ' ' +
				 	...
				 	$(opener.document).find('#mem_add2').focus();
				 	self.close();					
				});
			}
		});
